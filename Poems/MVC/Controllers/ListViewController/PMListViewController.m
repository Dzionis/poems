...
@interface PMListViewController ()<UITableViewDelegate,
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [self.navigationController.navigationBar
      setShadowImage:[UIImage imageFromColor:[theme grayColor]]];
}
  ...
  if (_fetchedResultsController == nil) {
    ...
    if (self.favorite) {
      fetchRequest.predicate =
          [NSPredicate predicateWithFormat:@"favorite = YES"];
    }
...
- (void)openFavorite:(UIBarButtonItem *)sender {
  PMListViewController *listViewController =
      [PMListViewController viewControllerFromStoryboard];
...
- (void)customizeSearchBar {
  ...
  [self.searchBar setImage:[theme imageSearch]
...
- (void)tableView:(UITableView *)tableView
  ...
  PMPoemPageViewController *poemPageViewController =
      [PMPoemPageViewController viewControllerFromStoryboard];
  poemPageViewController.title = poem.name;
  poemPageViewController.poem = poem;
  poemPageViewController.favorite = self.favorite;
  [self.navigationController pushViewController:poemPageViewController
                                       animated:YES];
}
...
- (UIView *)tableView:(UITableView *)tableView
  ...
  PMPoemListSectionView *sectionHeaderView = [[[NSBundle mainBundle]
      loadNibNamed:NSStringFromClass([PMPoemListSectionView class])
             owner:self
...
- (void)filterContentForSearchText:(NSString *)searchText {
  ...
  if (query && query.length) {
    if (self.favorite) {
      predicate = [NSPredicate predicateWithFormat:@"(name contains[cd] %@ or "
                                                   @"text contains[cd] %@) "
                                                   @"and favorite = YES",
                                                   query, query];
    } else {
      predicate = [NSPredicate
          predicateWithFormat:@"name contains[cd] %@ or text contains[cd] %@",
                              query, query];
    }
  } else {
    if (self.favorite) {
  ...
  [self.fetchedResultsController.fetchRequest
      setFetchLimit:100];  // Optional, but with large datasets - this helps
                           // speed lots
